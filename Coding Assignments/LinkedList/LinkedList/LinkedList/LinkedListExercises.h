...
void LinkedList<T>::insertOrdered(const T& newData) {
      ...
      // The Node already has next and prev set to nullptr by default.
        ...
        while (cur) {
            if (cur->data >= newNode->data) {
                if (cur == head_) {
                    //inserting at head with pushFront
                    pushFront(newData);
                    //printAddresses();
                    delete newNode;
                    return;
    ...
    //nothing inserted, insert at back
    pushBack(newData);
    delete newNode;
    return;
...
/*template <typename T>
void LinkedList<T>::printAddresses() {
    
    cout << "\nRECHECKING LIST" << endl;
    Node* cur = head_;
    cout << "head is " << head_ << endl;
    while (cur) {
        std::cout << "node " << cur->data << " at address " << cur << " between " << cur->prev << " and " << cur->next << std::endl;
        cur = cur->next;
    }
    cout << "tail is " << tail_ << endl;
}*/
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  LinkedList<T> merged;
    ...
    Node* trackLeft = left.head_; //points to the current pointer in left
    Node* trackRight = right.head_; //points to the current pointer in right
    
    if (!trackLeft || !trackRight) {//one of the lists is empty
    ...
    while (trackLeft || trackRight) { //while either one is non-null
        
        if (trackLeft && trackRight) { //if Left and Right are good, compare
                if (trackLeft->data <= trackRight->data) { //insert Left
                              merged.pushBack(trackLeft->data);
                              trackLeft = trackLeft->next;
                            }
                else { //or insert right
                    merged.pushBack(trackRight->data);
                    trackRight = trackRight->next;
                }
            }
        
        else if (!trackLeft) { //if left is done, push and walk right
            merged.pushBack(trackRight->data);
            trackRight = trackRight->next;
        }
        
        else { //else right is done, push and walk left
            merged.pushBack(trackLeft->data);
            trackLeft = trackLeft->next;
        }
    }
